...
impl Value {
    ...
    #[must_use]
    pub const fn new(node: Node<'static>) -> Self {
        Self(node)
    }
    ...
    #[must_use]
    pub const fn as_node(&self) -> &Node<'static> {
        &self.0
    }
    ...
    /// preserved verbatim from the source document. Glaucus never maps a tag to a
    ...
    #[must_use]
    pub const fn tag(&self) -> Option<&glaucus_core::types::Tag<'static>> {
        self.0.tag()
    }
}
...
const fn scalar_node(value: Cow<'static, str>, style: ScalarStyle) -> Node<'static> {
    Node::Scalar(Scalar {
        value,
        tag: None,
        style,
...
const fn plain_scalar(value: Cow<'static, str>) -> Node<'static> {
...
const fn quoted_scalar(value: Cow<'static, str>) -> Node<'static> {
...
/// # Errors
///
/// Returns an error if the round-trip through serde fails — either the
/// node has no YAML representation, or the re-parse rejects it.
...
/// # Errors
///
/// Returns an error if `node` cannot be represented as a [`Value`].
...
mod tests {
    ...
    /// double-quoted scalar. Drive the visitor method directly since glaucus's own
    ...
    fn roundtrip_value_through_yaml() {
        let original: Value = de::from_str("name: glaucus\nversion: 1").unwrap();
    ...
    fn visit_map_with_two_entries_pushes_each() {
        ...
        let de: MapDeserializer<'_, _, DeErr> =
    ...
    fn value_visitor_expecting_message() {
        ...
        let _ = fmt::Write::write_fmt(&mut buf, format_args!("{}", DummyDisplay(ValueVisitor)));
    ...
    fn value_preserves_custom_tag_via_node_path() {
        ...
        let node = glaucus_ast::composer::Composer::new("!mytag hello\n")
            .next()
            .unwrap()
            .unwrap()
            .into_owned();
