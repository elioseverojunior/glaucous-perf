...
const fn plain_static(value: &'static str) -> Node<'static> {
    Node::Scalar(Scalar {
...
/// # Errors
///
/// Returns an error if `value`'s `Serialize` implementation fails, or if a
/// value cannot be represented in YAML (e.g. a non-string map key in a
...
///
/// Returns an error if `value`'s `Serialize` implementation fails, or if a
/// value cannot be represented in YAML.
...
/// # Errors
///
/// Returns an error if `writer` fails, if `value`'s `Serialize`
/// implementation fails, or if a value cannot be represented in YAML.
...
pub fn to_writer_with<T: ser::Serialize + ?Sized, W: std::io::Write>(
    ...
    if let Some(io_err) = adapter.into_error() {
        return Err(<Error as ser::Error>::custom(io_err));
    }
    result
}
...
///
/// Returns an error if `value`'s `Serialize` implementation fails, or if a
/// value has no YAML `Node` representation.
...
impl ser::Serializer for NodeSerializer {
    ...
    fn serialize_newtype_struct<T: ser::Serialize + ?Sized>(
        ...
        let mut node = value.serialize(Self)?;
        match name {
...
impl ser::SerializeMap for MapBuilder {
    ...
    // serde's `SerializeMap` contract guarantees `serialize_key` runs first;
    // reaching this without a pending key is a caller bug, not bad data.
...
mod tests {
    ...
    fn serialize_small_int_widths() {
        ...
        assert_eq!(
            to_string(&170_141_183_460_469_231_731_687_303_715_884_105_727i128).unwrap(),
    ...
    fn serialize_small_uint_widths() {
        ...
        assert_eq!(
            to_string(&340_282_366_920_938_463_463_374_607_431_768_211_455u128).unwrap(),
    ...
    fn flow_seq_renders_flow_style() {
        ...
        let out = to_string(&FlowSeq(vec![1, 2, 3])).unwrap();
    ...
    fn lit_str_renders_literal_block() {
        ...
        let out = to_string(&LitStr("line1\nline2\n")).unwrap();
    ...
    fn fold_str_renders_folded_block() {
        ...
        let out = to_string(&FoldStr("a b c\n")).unwrap();
