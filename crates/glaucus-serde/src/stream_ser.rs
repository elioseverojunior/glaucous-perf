...
//!
//! Drives the push emitter sink ([`glaucus_ast::emitter::Emitter`]) directly from
//! serde data-model calls, with **no intermediate [`Node`](glaucus_ast::node::Node)
...
use glaucus_ast::emitter::{Emitter, EmitterConfig};
...
///
/// Mirrors [`glaucus_ast::emitter::emit`]: when `config.explicit_document` is set,
...
mod tests {
    ...
    fn same<T: Serialize>(v: &T) {
        ...
        let oracle = glaucus_ast::emitter::emit_to_string(&node, &EmitterConfig::default());
    ...
    fn config_threads_through() {
        ...
        let oracle = glaucus_ast::emitter::emit_to_string(&node, &config);
