...
fn main() {
    ...
    // `GitVersion.yml` decides what `gitversion` reports, so a change to it must
    // re-stamp the binary. Emitting ANY `rerun-if-changed` opts out of cargo's
    // default "re-run when anything in the package changed", and this file lives
    // at the workspace root -- outside the package -- so without this line it was
    // invisible to the freshness check entirely. Verified: setting
    // `next-version: 0.2.0` made `gitversion` report `0.2.0-1` while a rebuilt
    // binary still printed `v0.1.0-1`.
    println!("cargo:rerun-if-changed=../../GitVersion.yml");
}
...
///
/// The status check is scoped to the paths that actually determine the compiled
/// artefact. Unscoped, it reported `-dirty` for edits that cannot reach the
/// binary -- and inside a container it reported it *unconditionally*, which is
/// how a verified image build stamped `cc50abca0c3f-dirty` from a clean tree:
/// `docker/Dockerfile` deletes `rust-toolchain.toml` before building,
/// `.dockerignore` trims `docs/`, `scripts/` and root `*.md` out of the context
/// so git sees them as deleted, and the `target/` cache mount is untracked.
/// A suffix that is always present carries no information.
///
/// The `:/` prefix is git's "from the repository root" pathspec magic, needed
/// because cargo runs a build script with cwd set to its own package directory
/// -- a bare `crates` would resolve to `crates/glaucus/crates` and match
/// nothing. `rust-toolchain.toml` is deliberately absent from the list: the
/// container removes it on purpose, and `RUSTC_VERSION` already records which
/// compiler was actually used.
