...
`Cargo.toml`. It re-exports the focused Glaucus workspace crates
(`glaucus-core`, `glaucus-ast`, `glaucus-serde`, `glaucus-cst`, `glaucus-schema`)
...
| --------- | :-----: | ------------------------- | ------------ | ------------------------------------------------------------------- |
| (core)    | always  | `glaucus-core`              | —            | `scanner`, `parser`, `error`, `limits`, `types` re-exports          |
| `ast`     |   yes   | `glaucus-ast`               | —            | Node API (`from_str_node`, …), `composer`, `emitter`, `Node`        |
| `serde`   |   yes   | `glaucus-serde` + `ast`     | `ast`        | Serde API (`from_str`, `to_string`, …), `Value`, `SerdeError`       |
| `cst`     |   yes   | `glaucus-cst`               | —            | Lossless, comment-preserving editing via `cst::Document`            |
| `schema`  |   no    | `glaucus-schema` + `ast` + `cst` | `ast`, `cst` | Span-anchored JSON-Schema validation over the `Node` tree (`schema`) |

`glaucus-core` is always present (not feature-gated); it carries the shared
...
that enables it; `glaucus-core` is unconditional.

```mermaid
graph TD
    USER["Your Code"]
    GLAUCUS["glaucus<br/><i>facade crate</i>"]
    CORE["glaucus-core<br/><i>scanner · parser · always on</i>"]
    AST["glaucus-ast<br/><i>Node · composer · emitter</i>"]
    SERDE["glaucus-serde<br/><i>serde integration</i>"]
    CST["glaucus-cst<br/><i>lossless editing</i>"]
    SCHEMA["glaucus-schema<br/><i>JSON-Schema validation</i>"]

    USER --> GLAUCUS
    GLAUCUS --> CORE
    GLAUCUS -->|feature = ast| AST
    GLAUCUS -->|feature = serde| SERDE
    GLAUCUS -->|feature = cst| CST
    GLAUCUS -->|feature = schema| SCHEMA
...
let config: Config = glaucus::from_str("name: my-app\ndebug: true\nport: 8080").unwrap();
...
let docs: Vec<String> = glaucus::from_str_multi("---\nhello\n---\nworld\n").unwrap();
assert_eq!(docs, vec!["hello", "world"]);

// Node API: one Node per document.
let nodes = glaucus::from_str_multi_node("---\na\n---\nb\n").unwrap();
...
use glaucus::error::{ParserConfig, Strictness};
...
let yaml = glaucus::to_string_with(&Data { key: "val".into() }, &ecfg).unwrap();
...
reachable through the facade re-exports — `glaucus::scanner`, `glaucus::parser`,
`glaucus::composer`, `glaucus::emitter` — so you can work at whatever level of
control you need.

```mermaid
graph LR
    INPUT["&str"] --> SCAN["Scanner<br/><code>glaucus::scanner</code>"]
    SCAN -->|Token| PARSE["Parser<br/><code>glaucus::parser</code>"]
    PARSE -->|Event| COMPOSE["Composer<br/><code>glaucus::composer</code>"]
    COMPOSE --> NODE["Node Tree<br/><code>glaucus::Node</code>"]
    NODE -->|Deserializer| T["T / Value"]
    T -->|Serializer| NODE
    NODE --> EMIT["Emitter<br/><code>glaucus::emitter</code>"]
    EMIT --> OUTPUT["String"]
