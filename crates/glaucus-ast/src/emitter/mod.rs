...
//! use glaucus_ast::emitter::{emit_to_string, EmitterConfig};
//!
...
use glaucus_core::types::CollectionStyle;
...
/// # Panics
///
/// Cannot panic in practice: the only error [`emit`] reports is one from the
/// writer, and `fmt::Write` for `String` never fails. It is unwrapped rather
/// than discarded so that a future sink bug surfaces here instead of silently
/// returning truncated YAML.
#[must_use]
...
///
/// Layout lives entirely in the push [`Emitter`]; this is a thin
/// recursive walk (`drive_node`) that drives that sink. The optional
/// `explicit_document` markers (`---` / `...`) are written around the body.
///
/// # Errors
///
/// Propagates any error returned by `writer`. Emission itself is infallible —
/// a `Node` tree is already valid by construction — so every error originates
/// in the destination.
pub fn emit<W: fmt::Write>(node: &Node<'_>, config: &EmitterConfig, writer: &mut W) -> fmt::Result {
    ...
    {
        let mut sink = Emitter::new(WriteAdapter(writer), config);
...
const fn needs_quoted_escape(c: char) -> bool {
    matches!(c,
...
mod tests {
    ...
    use glaucus_core::types::{Position, ScalarStyle, Span, Tag};
