...
//! YAML test suite integration for glaucus.
//!
...
/// # Panics
///
/// Panics if `data_dir` cannot be read. Callers check that the submodule is
/// initialised before calling, so an unreadable directory means the checkout
/// is broken rather than that a test failed.
#[must_use]
...
/// Run the glaucus parser on input and return events in the test suite tree format.
///
/// # Errors
///
/// Returns the parser's error message if `input` is not valid YAML; the test
/// suite treats that as the expected outcome for its error cases.
...
fn event_to_tree_line(kind: &EventKind<'_>) -> String {
    match kind {
        ...
        EventKind::Scalar {
            ...
            let _ = write!(s, " {style_char}{}", escape_value(value));
            s
        }
...
pub fn show_diff(expected: &[String], actual: &[String]) -> String {
    ...
    for i in 0..max {
        let exp = expected.get(i).map_or("<missing>", String::as_str);
        let act = actual.get(i).map_or("<missing>", String::as_str);
        if exp != act {
            return format!(
                "  line {}: expected: {exp:?}\n  line {}:   actual: {act:?}",
                i + 1,
...
mod tests {
    ...
    impl TempDir {
        fn new(name: &str) -> Self {
            ...
            path.push(format!("glaucus_test_{name}_{}", std::process::id()));
    ...
    fn run_test_unexpected_parse_error_is_fail() {
        ...
        match run_test(&tc) {
            ...
            TestResult::Pass => {
                panic!("expected Fail for tab-in-block-mapping, got Pass")
            }
        }
    }
}
